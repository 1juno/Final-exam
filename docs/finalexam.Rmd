...
아래처럼 데이터 읽은 후, Q_로 시작하는 변수를 범주형으로 바꾼 후, 연도별 기술통계량을 Table로 나타내어라 (`jstable::CreateTableOneJS` 이용)
...
status ~ rx + age + sex 선형회귀 실행 후 Table로 나타내어라(`jstable::glmshow.display` 이용)
...
jskm::jskm(res.km, table = T, pval = T, label.nrisk = "No. at risk", size.label.nrisk = 8, xlabs = "Time(Day)", ylabs = "Survival", ystratalabs = c("Obs", "Lev", "Lev + 5FU"), ystrataname = "rx", marks = F, timeby = 365, xlims = c(0, 3000), ylims = c(0.25, 1), showpercent = T)
```